    ...
    private void updateReturnStatus(boolean returned) {
        ...
        if (selectedRow != -1) {
            try (Connection connection = new Db().getConnection()) {
                ...
                String query = "SELECT p.fecha_prestamo, p.fecha_devolucion "
                             + "FROM prestamos p "
                ...
                try (PreparedStatement statement = connection.prepareStatement(query)) {
                    ...
                    try (ResultSet resultSet = statement.executeQuery()) {
                        if (resultSet.next()) {
                            ...
                            if (fechaDevolucion != null) {
                                JOptionPane.showMessageDialog(this, "Este libro ya ha sido devuelto.",
                                        "Información", JOptionPane.INFORMATION_MESSAGE);
                                return;
                            }

                            // Calcular multa (ejemplo: 1.5 unidades monetarias por día de retraso)
                            long diferenciaDias = (new Date(System.currentTimeMillis()).getTime() - fechaPrestamo.getTime()) / (1000 * 60 * 60 * 24);
                            double multa = 0;
                            ...
                            try (PreparedStatement updateStatement = connection.prepareStatement(updatePrestamoQuery)) {
                                ...
                                if (updatedRows > 0) {
                                    JOptionPane.showMessageDialog(this, "Estado de devolución y multa actualizados con éxito.",
                                            ...
                                            try (ResultSet waitlistResult = waitlistStatement.executeQuery()) {
                                                if (waitlistResult.next()) {
                                                    ...
                                                    try (PreparedStatement prestamoStatement = connection.prepareStatement(prestamoQuery)) {
                                                        ...
                                                        prestamoStatement.setDate(3, new Date(System.currentTimeMillis()));
                                                        ...
                                                        try (PreparedStatement eliminarEsperaStatement = connection.prepareStatement(eliminarEsperaQuery)) {
                                                    ...
                                                    JOptionPane.showMessageDialog(this, "El libro ha sido devuelto y está disponible nuevamente.",
                                                            "Éxito", JOptionPane.INFORMATION_MESSAGE);
                                                }
                                            }
                                        }
                ...
                ex.printStackTrace();
                JOptionPane.showMessageDialog(this, "Error al actualizar el estado: " + ex.getMessage(),
                        "Error", JOptionPane.ERROR_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(this, "Por favor, seleccione un préstamo para actualizar.",
                    "Advertencia", JOptionPane.WARNING_MESSAGE);
        }
    }
                    ...
                    try (PreparedStatement checkStatement = connection.prepareStatement(disponibilidadQuery)) {
                        ...
                        try (ResultSet resultSet = checkStatement.executeQuery()) {
                            if (resultSet.next() && resultSet.getInt("PrestamosActivos") > 0) {
                                ...
                                try (PreparedStatement waitlistStatement = connection.prepareStatement(listaEsperaQuery, Statement.RETURN_GENERATED_KEYS)) {
                                    waitlistStatement.setString(1, titulo);
                                    ...
                                    try (PreparedStatement posicionStatement = connection.prepareStatement(posicionQuery)) {
                                        ...
                                        try (ResultSet posicionResult = posicionStatement.executeQuery()) {
                                            if (posicionResult.next()) {
                                                ...
                                                JOptionPane.showMessageDialog(dialog,
                                                        ...
                                                        "Se ha agregado a la lista de espera en la posición: " + posicion,
